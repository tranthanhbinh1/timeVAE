...
class VariationalAutoencoderConvInterpretable(BaseVariationalAutoencoder):
    ...
    def scale_model(self, z):
        ...
        scale_params = Reshape(target_shape=(1, self.feat_dim))(
            scale_params
        )  # shape: (N, 1, D)

        scale_vals = tf.repeat(
            scale_params, repeats=self.seq_len, axis=1
        )  # shape: (N, T, D)
    ...
    def trend_model(self, z):
        trend_params = Dense(
            self.feat_dim * self.trend_poly, name="trend_params", activation="relu"
        )(z)
        trend_params = Dense(self.feat_dim * self.trend_poly, name="trend_params2")(
            trend_params
        )
        trend_params = Reshape(target_shape=(self.feat_dim, self.trend_poly))(
            trend_params
        )  # shape: N x D x P
        ...
        lin_space = (
            K.arange(0, float(self.seq_len), 1) / self.seq_len
    ...
    def _get_season_indexes_over_seq(self, num_seasons, len_per_season):
        ...
        while curr_len < self.seq_len:
            reps = (
                len_per_season
                if curr_len + len_per_season <= self.seq_len
                else self.seq_len - curr_len
            )
            season_idx.extend([curr_idx] * reps)
            curr_idx += 1
            if curr_idx == num_seasons:
                curr_idx = 0
            curr_len += reps
        return season_idx
    ...
    def generic_seasonal_model(self, z):
        freq = Dense(
            self.feat_dim * self.num_gen_seas,
            name="g_season_freq",
            activation="sigmoid",
        )(z)
        freq = Reshape(target_shape=(1, self.feat_dim, self.num_gen_seas))(
            freq
        )  # shape: (N, 1, D, S)

        phase = Dense(self.feat_dim * self.num_gen_seas, name="g_season_phase")(z)
        phase = Reshape(target_shape=(1, self.feat_dim, self.num_gen_seas))(
            phase
        )  # shape: (N, 1, D, S)

        amplitude = Dense(self.feat_dim * self.num_gen_seas, name="g_season_amplitude")(
            z
        )
        amplitude = Reshape(target_shape=(1, self.feat_dim, self.num_gen_seas))(
            amplitude
        )  # shape: (N, 1, D, S)
        ...
        lin_space = tf.reshape(
            lin_space, shape=(1, self.seq_len, 1, 1)
        )  # shape: 1, T, 1, 1
        # print('lin_space:', lin_space)

        seas_vals = amplitude * K.sin(
            2.0 * np.pi * freq * lin_space + phase
        )  # shape: N, T, D, S
    ...
    def generic_seasonal_model2(self, z):
        season_params = Dense(
            self.feat_dim * self.num_gen_seas, name="g_season_params"
        )(z)
        season_params = Reshape(target_shape=(self.feat_dim, self.num_gen_seas))(
            season_params
        )  # shape: (D, S)
        ...
        ls = (
            K.arange(0, float(self.seq_len), 1) / self.seq_len
        )  # shape of ls : 1d tensor of length T
